...
UCLASS(Abstract)
class DEHRGADATWU_API AFogOfWarManager : public AActor
{
	GENERATED_BODY()
...
public:
	...
	UFUNCTION(BlueprintNativeEvent, Category = FogOfWar)
		void OnFowTextureUpdated(UTexture2D* currentTexture, UTexture2D* lastTexture);

	void RegisterFowViewer(class UFogOfWarViewer* viewer);
	void RegisterFowLight(class UFogOfWarLight* light);
	void UpdateStaticLight(class UFogOfWarLight* light, bool add);
	...
	//How clear should previously explored areas be when out of sight
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = FogOfWar, meta = (ClampMin = 0, ClampMax = 1))
		float partialFog = .1f;
	...
	UPROPERTY()
		TArray<float> SightData;
	UPROPERTY()
		TArray<float> LightData;
	UPROPERTY()
		TArray<float> StaticLightData;
	UPROPERTY()
		TArray<bool> seenThisCycleData;
	...
	UPROPERTY()
		TArray<class UFogOfWarViewer*> FowViewers;
	UPROPERTY()
		TArray<class UFogOfWarLight*> FowLights;
